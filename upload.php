...
function upload_image(){
    ...
    if (in_array($file_ext,$allowed_file_types) && ($filesize < 200000))
    {
        ...
        if (file_exists("uploads/images/" . $newfilename))
        {
            ...
            $imageInformation = getimagesize($_FILES["image"]["tmp_name"]);
...
function upload_logo()
{
    ...
    if (in_array($file_ext,$allowed_file_types) && ($filesize < 200000))
    {
        ...
        if (file_exists("uploads/logo/" . $newfilename))
        {
            ...
            $imageInformation = getimagesize($_FILES['logo']['tmp_name']);
            //print_r($imageInformation);
            ...
            if ($imageWidth <= 900 && $imageHeight <= 900) {
                ...
                move_uploaded_file($_FILES["logo"]["tmp_name"], "uploads/logo/" . $newfilename);
                mysqli_select_db($GLOBALS['db1'], "jobportal");
                $cmd = mysqli_query($GLOBALS['db1'], "update company set logo = '$newfilename' WHERE eid=$_SESSION[eid]");
                if (!$cmd) {
                    echo("Error description: " . mysqli_error($db1));
                } else {
                    //echo "File uploaded succesfully ; <a href='company/profile.php'> Go back to profile </a>";
                    header('location:company/profile.php?msg=suc-logo');
                }
            } else{
                echo "image resoluion is too large";
            }
